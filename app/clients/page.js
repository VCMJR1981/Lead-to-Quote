...
export default function ClientsPage() {
  ...
  leads.forEach(lead => {
    ...
    if (!clientMap[key]) { clientMap[key] = { latest: lead, allJobs: [lead] } }
    else {
      clientMap[key].allJobs.push(lead)
      if (new Date(lead.created_at) > new Date(clientMap[key].latest.created_at)) clientMap[key].latest = lead
    }
  })
  const clients = Object.values(clientMap).sort((a, b) => new Date(b.latest.created_at) - new Date(a.latest.created_at))
  ...
  const totalWon = leads.filter(l => l.status === 'won').reduce((s, l) => s + (l.quotes?.[0]?.total || 0), 0)
    ...
    <div className="min-h-screen bg-[#FAFAF9]">
      <div className="bg-white border-b border-gray-100 px-4 pt-14 pb-4 sticky top-0 z-10">
        ...
        <div className="relative">
          <svg className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-500" width="16" height="16" fill="none" viewBox="0 0 16 16"><path d="M7 12A5 5 0 107 2a5 5 0 000 10zM14 14l-3-3" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/></svg>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5 flex-wrap">
                      <span className="text-xs font-mono text-gray-500">#{clientNum}</span>
                      ...
                      {prevJobs > 0 && (
                        <button onClick={e => { e.stopPropagation(); setExpanded(isExpanded ? null : lead.id) }}
                          className="text-xs px-2 py-0.5 rounded-full bg-purple-50 text-purple-700 font-medium flex items-center gap-1">
                          🔁 {prevJobs} prev {prevJobs === 1 ? 'job' : 'jobs'} {isExpanded ? '▲' : '▼'}
                        </button>
                      )}
                    </div>
                    <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1">
                      {lead.phone && <span className="text-xs text-gray-600">📞 {lead.phone}</span>}
                      {lead.email && <span className="text-xs text-gray-600 truncate max-w-[180px]">✉️ {lead.email}</span>}
                    </div>
                    {lead.job_type && <p className="text-xs text-gray-600 mt-1">{lead.job_type}</p>}
                    <div className="flex items-center justify-between mt-2">
                      <span className="text-xs text-gray-500">{timeAgo(lead.created_at)}</span>
                      <div className="text-right">
                        {totalSpent > 0 && <p className="text-sm font-bold font-heading text-green-600">{fmt(totalSpent)} total</p>}
                        {quote?.total > 0 && lead.status !== 'won' && <p className="text-sm font-bold font-heading text-gray-900">{fmt(quote.total)}</p>}
                      </div>
                    </div>
                  </div>
                  <button onClick={e => deleteLead(lead.id, e)} className="p-1.5 text-gray-400 hover:text-red-400 flex-shrink-0">
                    <svg width="14" height="14" fill="none" viewBox="0 0 14 14"><path d="M2 3.5h10M5.5 3.5V2.5h3v1M3.5 3.5l.5 8h6l.5-8" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  </button>
                </div>
              </div>

              {/* Previous jobs dropdown */}
              {isExpanded && prevJobs > 0 && (
                <div className="px-4 pb-4 pt-0 border-t border-gray-50">
                  <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide py-2">Previous jobs</p>
                  <div className="space-y-2">
                    {allJobs.filter(j => j.id !== lead.id).sort((a,b) => new Date(b.created_at) - new Date(a.created_at)).map(job => {
                      const jst = ST[job.status] || ST.new
                      return (
                        <div key={job.id} onClick={() => router.push(`/lead/${job.id}`)}
                          ...
                          <div className="flex items-center gap-2">
                            {job.quotes?.[0]?.total > 0 && <span className="text-xs font-semibold text-gray-700">{fmt(job.quotes[0].total)}</span>}
                            <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: jst.bg, color: jst.c }}>{job.status}</span>
                          </div>
                        </div>
                      )
                    })}
